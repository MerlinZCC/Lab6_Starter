...
function initFormHandler() {
  ...
  buttonEl.addEventListener("click", submitFunc)
  // Steps B4-B9 will occur inside the event listener from step B3
  function submitFunc (event){
    event.preventDefault();
  // B4. TODO - Create a new FormData object from the <form> element reference above
  let formD = new FormData(formEl);
  // B5. TODO - Create an empty object (I'll refer to this object as recipeObject to
  //            make this easier to read), and then extract the keys and corresponding
  //            values from the FormData object and insert them into recipeObject
  let recipeObject = {};
  for(const [key, value] of formD.entries()){
  ...
  let recipes = getRecipesFromStorage();
  recipes.push(recipeEl);
  saveRecipesToStorage(recipes);
  }
  // B10. TODO - Get a reference to the "Clear Local Storage" button
  const clearEl = document.getElementsByTagName('button')[1];
  // B11. TODO - Add a click event listener to clear local storage button
  clearEl.addEventListener("click", clearFunc)
